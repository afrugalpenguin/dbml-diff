...
- **A large `--format mermaid` diff renders correctly but too small to read, and the cause is the missing relationships.** With no `Ref:` lines there are no edges, so the layout engine has nothing to stack against and lays every entity out in a single row: width grows linearly with the number of changed tables while height stays flat, and the renderer then scales that width down to fit its container. Mermaid's `er.layoutDirection` and `er.nodeSpacing` are ignored by its ER renderer, so an `%%{init}%%` directive cannot fix it, and `--hide-unchanged-pk` only removes rows, which is the wrong axis. The only lever is fewer entities per diagram. This makes `--format mermaid` a fit for the size of a single change - a pull request - rather than a release; `--format text` and `--format json` stay readable at any size.
