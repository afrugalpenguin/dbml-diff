...
function parseSchema(text) {
  ...
  for (const schema of db.schemas) {
    ...
    for (const r of (schema.refs || [])) {
      ...
      // Key by the set of (fromCol -> toCol) pairs, not their positional order:
      // a composite ref listed as (x,y)>(x,y) and (y,x)>(y,x) is the same FK.
      // Re-pairing one side (e.g. (y,x)>(x,y)) changes the pairs and stays a change.
      const pairKey = from.columns
        .map((c, i) => `${c}:${to.columns[i]}`)
        .sort()
        .join(',');
      const key = `${from.table} > ${to.table} [${pairKey}]`;
      refs.set(key, {
