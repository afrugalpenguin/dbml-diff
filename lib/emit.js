...
// Mermaid's grammar is far stricter than DBML's, measured against mermaid
// 11.16.0:
//   - entity names must be quoted, and then hold spaces, dots, `·` and unicode
//     happily - but a double quote has no escape at all;
//   - attribute comments are quoted with the same no-escape rule;
//   - attribute types and names must be a single bare token starting with a
//     letter or underscore: no spaces, no `·`, no `$`/`%`, no leading digit.
// All three are reachable from real DBML (`c "character varying(50)"`,
// `"my col" int`, `"2fa" bool`, a note containing a quote), so both sanitisers
// below are load-bearing, not defensive padding.
//
// Commas are excluded from identifiers even though mermaid 11.16.0 accepts
// them: an older renderer rejects `DECIMAL(18,2)` outright and drops the whole
// diagram, and renderers pin their own mermaid version (GitHub and Azure DevOps
// both do). `DECIMAL(18_2)` parses everywhere tested, so the emitter targets the
// strict intersection rather than the newest grammar. A comma inside a *comment*
// is fine and is left alone.
const MM_ILLEGAL = /[^\p{L}\p{N}_\-[\]()]/gu;
/** Fold a value into a legal mermaid attribute type/name token. */
const mmIdent = (s) => {
  const t = String(s).replace(MM_ILLEGAL, '_');
  return /^[\p{L}_]/u.test(t) ? t : `_${t}`;
};
/** Fold a value into a legal mermaid quoted string (entity name or comment). */
const mmText = (s) => String(s).replace(/"/g, "'").replace(/\s+/g, ' ').trim();
/** Open a mermaid entity block. */
const mmEntity = (name) => `  "${mmText(name) || '(unnamed)'}" {`;

/**
 * Render one mermaid attribute line: `type name [PK] ["comment"]`. Mermaid has
 * no NOT NULL concept, so nullability rides in the comment rather than being
 * dropped.
 */
function mmColumn(name, type, opts = {}) {
  let line = `    ${mmIdent(type)} ${mmIdent(name)}`;
  if (opts.pk) line += ' PK';
  const note = [opts.note, opts.notNull && !opts.pk ? 'not null' : null]
    .filter(Boolean).join(' · ');
  if (note) line += ` "${mmText(note)}"`;
  return line;
}

/**
 * Render a diff result as a Mermaid `erDiagram` block, so it renders natively
 * wherever Mermaid does (GitHub, Azure DevOps) with nothing to install.
 *
 * This is the same information `emitDbml` renders, in a different notation: no
 * refs (the diff does not carry cardinality, so they stay counts) and no new
 * inference. The block is emitted bare, without a markdown fence - wrapping is
 * the embedder's job.
 * @param {DiffResult} result
 * @param {Object} [opts]
 * @param {string} [opts.oldLabel='old'] Label for the old schema in the header
 ...
 *   tables instead of a PK stub with a column count
 * @param {boolean} [opts.hideUnchangedPk=false] Suppress the unchanged
 *   primary-key orientation row in MOD entities
 * @param {Date|string} [opts.date] Generation date for the header (defaults to today)
 * @returns {string} Mermaid source
 */
function emitMermaid(result, opts = {}) {
  const oldLabel = opts.oldLabel || 'old';
  const newLabel = opts.newLabel || 'new';
  const fullNewTables = !!opts.fullNewTables;
  const hideUnchangedPk = !!opts.hideUnchangedPk;
  ...
  const L = [];
  // Separate the header with a blank line, never a bare `%%`: mermaid strips
  // comments with `%%[^\n]+`, which needs at least one character after the `%%`,
  // so an empty `%%` line survives cleanup and then fails the grammar.
  ...
  // The dbml summary encodes each metric as label-as-name + count-as-type, which
  // mermaid rejects twice over (no quoted attribute names, no type starting with
  // a digit). Its `type name "comment"` grammar gives the same numbers a cleaner
  // home: category, action, count - all three render as columns.
  L.push(mmEntity(`DIFF SUMMARY · ${oldLabel} -> ${newLabel}`));
  const row = (cat, action, n) => `    ${mmIdent(cat)} ${mmIdent(action)} "${n}"`;
  const rowIf = (cat, action, n) => { if (n) L.push(row(cat, action, n)); };
  L.push(row('Tables', 'added', added.length),
    row('Tables', 'removed', removed.length),
    row('Tables', 'modified', modified.length));
  rowIf('Enums', 'added', enums.added.length);
  ...
  if (modified.length) {
    L.push(`%% ----- MODIFIED TABLES -----`);
    for (const m of modified) {
      L.push(mmEntity(`${PREFIX.modified}${m.name}`));
      const pk = m.columns.find((c) => c.pk);
      if (pk && !hideUnchangedPk) L.push(mmColumn(pk.name, pk.type, { pk: true, note: 'unchanged columns omitted' }));
      for (const c of m.columnsAdded)
        L.push(mmColumn(`${c.name}__ADDED`, c.type, { notNull: c.notNull, note: 'ADDED' }));
      for (const c of m.columnsRemoved)
        L.push(mmColumn(`${c.name}__REMOVED`, c.type, { note: 'REMOVED' }));
      for (const r of m.renames)
        L.push(mmColumn(`${r.to.name}__RENAMED`, r.to.type, { notNull: r.to.notNull, note: `RENAMED from ${r.from.name} (heuristic - verify)` }));
      for (const ch of m.columnsChanged)
        L.push(mmColumn(`${ch.column.name}__CHANGED`, ch.column.type, { notNull: ch.column.notNull, note: `CHANGED: ${ch.changes.join('; ')}` }));
      L.push(`  }`);
    }
  }
  ...
  if (added.length) {
    L.push(`%% ----- ADDED TABLES -----`);
    for (const t of added) {
      L.push(mmEntity(`${PREFIX.added}${t.name}`));
      if (fullNewTables) {
        let first = true;
        for (const c of t.columns) {
          L.push(mmColumn(c.name, c.type, { pk: c.pk, notNull: c.notNull, note: first ? 'NEW TABLE' : c.note }));
          first = false;
        }
      } else {
        const stub = t.columns.find((c) => c.pk) || t.columns[0];
        if (stub) L.push(mmColumn(stub.name, stub.type, { pk: stub.pk, notNull: stub.notNull, note: `NEW TABLE - ${plural(t.columns.length, 'column')}` }));
      }
      L.push(`  }`);
    }
  }
  ...
  if (removed.length) {
    L.push(`%% ----- REMOVED TABLES -----`);
    for (const t of removed) {
      L.push(mmEntity(`${PREFIX.removed}${t.name}`));
      let first = true;
      for (const c of t.columns) {
        L.push(mmColumn(c.name, c.type, { pk: c.pk, notNull: c.notNull, note: first ? 'TABLE REMOVED' : null }));
        first = false;
      }
      L.push(`  }`);
    }
  ...
  // Mermaid has no enum construct, so a changed enum renders as an entity with
  // its values as attributes, marked `(enum)` to keep it distinct from a table.
  if (enumsChanged) {
    L.push(`%% ----- ENUMS -----`);
    for (const en of enums.modified) {
      L.push(mmEntity(`${PREFIX.modified}${en.name} (enum)`));
      const addedSet = new Set(en.valuesAdded);
      for (const v of en.values) L.push(mmColumn(v, 'value', { note: addedSet.has(v) ? 'ADDED' : null }));
      for (const v of en.valuesRemoved) L.push(mmColumn(v, 'value', { note: 'REMOVED' }));
      L.push(`  }`);
    }
    for (const en of enums.added) {
      L.push(mmEntity(`${PREFIX.added}${en.name} (enum)`));
      let first = true;
      for (const v of en.values) { L.push(mmColumn(v, 'value', { note: first ? 'NEW ENUM' : null })); first = false; }
      L.push(`  }`);
    }
    for (const en of enums.removed) {
      L.push(mmEntity(`${PREFIX.removed}${en.name} (enum)`));
      let first = true;
      for (const v of en.values) { L.push(mmColumn(v, 'value', { note: first ? 'ENUM REMOVED' : null })); first = false; }
      L.push(`  }`);
    }
  }
...
module.exports = { emitDbml, emitMermaid, emitText, emitJson, emitMigration, PREFIX, COLORS };
