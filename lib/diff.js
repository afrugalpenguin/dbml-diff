...
/**
 * Per-category change counts for a {@link DiffResult}, plus their total. Every
 * "did anything change / how many" decision - each emitter's section guards and
 * the CLI's summary line and exit code - derives from this one place, so the
 * counts cannot drift between call sites as new categories are added.
 * @param {DiffResult} result
 * @returns {{ tables: number, enums: number, refs: number, groups: number, total: number }}
 */
function changeCounts(result) {
  const { counts, enums, refs, groups } = result;
  const tables = counts.added + counts.removed + counts.modified;
  const e = enums.added.length + enums.removed.length + enums.modified.length;
  const r = refs.added.length + refs.removed.length +
    refs.retargeted.length + refs.unresolved.length;
  const g = groups.added.length + groups.removed.length + groups.modified.length;
  return { tables, enums: e, refs: r, groups: g, total: tables + e + r + g };
}
 ...
 * @returns {DiffResult}
 */
function diffTables(oldTables, newTables, opts = {}) {
  const added = [];
  const removed = [];
  const modified = [];
  ...
  for (const [name, nt] of newTables) {
    ...
    for (const [cn, nc] of nt.cols) {
      ...
      if (opts.includeNotes && (oc.note || null) !== (nc.note || null))
 ...
 * @param {Object} [opts]
 * @param {boolean} [opts.includeNotes=false] Treat a changed column `note` as a
 *   column change (`note changed`). Off by default - notes are noise in most diffs.
 * @returns {DiffResult}
 */
function diffSchemas(oldSchema, newSchema, opts = {}) {
  const t = diffTables(oldSchema.tables, newSchema.tables, opts);
  return {
    tables: t.tables,
 ...
 * @param {Object} [opts] See {@link diffSchemas}
 * @returns {DiffResult}
 */
function diff(oldDbml, newDbml, opts = {}) {
  return diffSchemas(parseSchema(oldDbml), parseSchema(newDbml), opts);
}

module.exports = { diff, diffTables, diffEnums, diffRefs, diffGroups, diffSchemas, changeCounts };
